...
def edit_movie():
    title = input("Enter the movie title to edit ✏️: ")
...
def delete_movie():
    title = input("🗑️ Enter the movie title to delete: ")
    if title in movie_database:
        del movie_database[title]
        print(f"{title} has been deleted from the database.")
    else:
        print(f"⚠️ {title} is not in the database.")
...
while True:
    ...
    print("2. ✏️ Edit an existing movie")
    print("3. 🗑️ Delete a movie")
    print("4. 👁️ View all movies")
    ...
    if choice == "1":
        ...
        print("👋 Goodbye! Thanks for using the Movie App 🍿")
        exit()
    else:
